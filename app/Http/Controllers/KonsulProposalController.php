...
class KonsulProposalController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($status_dospem) {
            /* Return menuju view */
            return view('mahasiswa.konsultasi.proposal.index', compact('tahun_id','detail'));
        } else {
            /* Return menuju view */
            return view('mahasiswa.konsultasi.proposal.alihkan');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if(!$validator->passes()){
            ...
            if (!$cek_kondisi || $cek_kondisi->status != "Disetujui" && $cek_kondisi->status != "Selesai") {
                $data = "Konsultasi judul Anda belum selesai, silahkan lanjut untuk konsultasi judul hingga berstatus 'disetujui untuk pengujian'!";
                ...
                if ($get_status && $get_status->status == "Selesai"){
                    ...
                    if ($get_status && !$get_status->tanggapan) {
                        ...
                        File::delete('dokumen/konsultasi/proposal/'.$bimbingan->file_upload);
                        ...
                        // /* Notifikasi email */
                        // $subjek = 'Konsultasi Proposal Terbaru';
                        // $details = [
                        //     'title' => 'Konsultasi Proposal dari Mahasiswa Bimbingan Anda',
                        //     'body' => 'Anda menerima konsultasi proposal terbaru dari mahasiswa yang bernama ' . $user->mahasiswa->nama_mahasiswa
                        // ];
    ...
    public function show($konsultasi_propsal)
    {
        /* Ambil data materi dosen sesuai parameter */
        $data = RiwayatBimbinganModel::find($konsultasi_propsal)->load('bimbingan');

        /* Return json data materi tahunan */
        return response()->json($data);
    }

    public function update(Request $request, $konsultasi_proposal)
    {
    ...
    public function storeKomen(Request $request)
    {
        ...
        if(!$validator->passes()){
            ...
            if ($get_status && $get_status->status == "Selesai"){
                ...
                // /* Notifikasi email */
                // $subjek = 'Komentar Konsultasi Proposal Terbaru';
                // $details = [
                //     'title' => 'Komentar Untuk Konsultasi Proposal dari Mahasiswa Bimbingan Anda',
                //     'body' => 'Anda menerima komentar untuk konsultasi proposal terbaru dari mahasiswa yang bernama ' . $user->mahasiswa->nama_mahasiswa
                // ];
    ...
    public function cetakPdf()
    {
    	...
    	$pdf = PDF::loadview('/mahasiswa/konsultasi/proposal/proposal_pdf', compact('user','riwayat','hari'));
    	return $pdf->stream();
    }
}
