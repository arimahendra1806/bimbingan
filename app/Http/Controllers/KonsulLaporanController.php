...
class KonsulLaporanController extends Controller
{
    ...
    public function storeKomen(Request $request)
    {
        ...
        if(!$validator->passes()){
            ...
            if ($get_status && $get_status->status == "Selesai"){
                ...
                $nomor = '62' . $user->mahasiswa->dospem->dosen->no_telepon;
                $pesan = 'Anda menerima komentar untuk konsultasi laporan terbaru dari mahasiswa yang bernama ' . $user->mahasiswa->nama_mahasiswa;
    ...
    public function cetakPdf()
    {
        ...
        $riwayat = RiwayatBimbinganModel::where('bimbingan_kode', $get_bimbingan->kode_bimbingan)
            ->where(function ($q){
                $q->where('bimbingan_jenis', 'Laporan')->orWhere('bimbingan_jenis', 'Program');
            })->get();
