        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Data Konsultasi Judul</h5>
                        ...
                        <div class="collapse show" id="collapseCetak">
                            <div class="card-body" style="text-align: justify">
                                @if ($detail['status_konsultasi'] == 'Disetujui')
                                    <p>
                                        Konsultasi judul sudah disetujui untuk pengujian, silahkan perbarui judul Anda
                                        apabila terdapat perubahan.
                                        <u><a href="{{ route('pengajuan-judul.index') }}">Perbarui Judul</a></u>.
                                    </p>
                                @elseif ($detail['status_konsultasi'] == 'Selesai')
                                    <p>
                                        Selamat konsultasi judul Anda sudah selesai.
                                    </p>
                                @endif
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var tableRiwayat = $('#RiwayatTabels').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('bimbingan-judul.riwayat') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
            ...
            $('body').on('click', '#btnEdit', function() {
                ...
                $.get('konsultasi-judul/show/' + this_id, function(data) {
                    $('#ModalEdit').modal('show');
                    $('#id_edit').val(data.id);
                    $('#fileShow').val(data.bimbingan.file_upload);
                    $('#keterangan_edit').val(data.keterangan);
                    $('iframe').attr("src", "{{ asset('dokumen/konsultasi/judul') }}" + "/" + data
                        .bimbingan.file_upload);
                });
            });
