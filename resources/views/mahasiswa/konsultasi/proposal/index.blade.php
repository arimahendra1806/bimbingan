        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Data Konsultasi Proposal</h5>
                        ...
                        <div class="collapse show" id="collapseCetak">
                            <div class="card-body" style="text-align: justify">
                                @if ($detail['status_konsultasi'] == 'Disetujui')
                                    <p>
                                        Konsultasi proposal sudah disetujui untuk pengujian, silahkan cetak kartu konsultasi
                                        melalui berikut
                                        <u><a href="{{ route('kartu-proposal.cetak') }}">Cetak Sekarang</a></u>.
                                    </p>
                                @elseif ($detail['status_konsultasi'] == 'Selesai')
                                    <p>
                                        Selamat konsultasi proposal Anda sudah selesai.
                                    </p>
                                @endif
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var tableRiwayat = $('#RiwayatTabels').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('bimbingan-proposal.riwayat') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
            ...
            $('body').on('click', '#btnEdit', function() {
                ...
                $.get('konsultasi-proposal/show/' + this_id, function(data) {
                    $('#ModalEdit').modal('show');
                    $('#id_edit').val(data.id);
                    $('#fileShow').val(data.bimbingan.file_upload);
                    $('#keterangan_edit').val(data.keterangan);
                    $('iframe').attr("src", "{{ asset('dokumen/konsultasi/proposal') }}" + "/" +
            ...
            $("#FormEdit").submit(function(e) {
                ...
                $.ajax({
                    url: "konsultasi-proposal/" + this_id,
                    type: "POST",
                    data: formData,
                    cache: false,
                    contentType: false,
